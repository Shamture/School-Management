     ...
     <div class="container custom-section custom-height">
       ...
       <div class="col-md-11">
          <table class="table">
            ...
            <c:forEach items = "${requestScope.listEleve }" var="item">
              <tr>
                <td> <c:out value="${item.nom }" /> </td>
                <td><c:out value="${item.prenom }" /></td>
                <td> <a href="<c:url value="/prof/postuler_remarque?idEleve=${item.idEleve }"/>" target="_blank" ><button class="btn btn-primary">Postuler Remarque</button> </a></td>
                <td><a href="<c:url value="/prof/postuler_absence?idEleve=${item.idEleve }"/>" target="_blank"><button class="btn btn-danger">Postuler Absence</button> </a></td>
              </tr>
            </c:forEach>
          </table>
       </div>
